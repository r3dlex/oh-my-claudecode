...
describe('plugin-setup.mjs hook command portability', () => {
    ...
    it('recognizes the committed quoted /bin/sh+find-node+run.cjs hook command format', () => {
        ...
        const currentCommand = '"/bin/sh" "$CLAUDE_PLUGIN_ROOT"/scripts/find-node.sh "$CLAUDE_PLUGIN_ROOT"/scripts/run.cjs "$CLAUDE_PLUGIN_ROOT"/scripts/keyword-detector.mjs';
